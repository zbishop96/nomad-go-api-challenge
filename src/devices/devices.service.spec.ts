...
describe('DevicesService', () => {
  ...
  it('Should reject a device with an invalid ID', async () => {
    await expect(
      service.enroll({ ...testValidDevice, id: '' as UUID }),
    ).rejects.toThrow();
    await expect(
      service.enroll({ ...testValidDevice, id: '12345' as UUID }),
    ).rejects.toThrow();
  });

  it('Should reject a device with invalid latitude or longitude', async () => {
    await expect(
      service.enroll({ ...testValidDevice, latitude: 100 }),
    ).rejects.toThrow();
    await expect(
      service.enroll({ ...testValidDevice, longitude: 300 }),
    ).rejects.toThrow();
  });
